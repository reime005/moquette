...
final class BrokerInterceptor implements Interceptor {
    ...
    @Override
    public void notifyClientConnected(final ConnectMessage msg, final ConnectIpPortMessage ipPortMessage) {
        for (final InterceptHandler handler : this.handlers) {
            executor.execute(new Runnable() {
                @Override
                public void run() {
                    handler.onConnect(new InterceptConnectMessage(msg, ipPortMessage));
                }
            });
        }
    }
